...
import boxDData from "../data/boxD.json";
import "./BoxPageD.css";

const BoxPageD = () => {
  const { addToCart } = useCart(); // ✅ Access addToCart from global context
  const product = boxDData[0];
  ...
  // ✅ Get price based on selected size from product.rates
  ...
  const handleAddToCart = () => {
    const cartItem = {
      ...
      price: price,           // ✅ numeric price for cart math
      displayPrice: rawPrice, // (optional) original price format
      size: selectedSize,
      color: null,
      thickness: null,
      length: null,
