...
const PipeBendPage = () => {
  ...
  // ✅ Calculate Price based on bend and size
  const price = product.rates[selectedBend][selectedSize];
  ...
  const handleAddToCart = () => {
    ...
    addToCart(cartItem);
    alert(`${product.name} (${selectedSize}, ${selectedBend} Bend) added to cart 🛒`);
  };
        ...
        <div className="right-info">
          ...
          <div className="options">
            {["1.5", "2.0"].map((bend) => (
              <button
                key={bend}
                className={selectedBend === bend ? "active" : ""}
